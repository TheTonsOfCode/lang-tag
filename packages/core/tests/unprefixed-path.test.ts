...
const translationTransformer: TranslationTransformer<
    LangTagTranslationsConfig
> = ({ value }) => value;
...
describe('createCallableTranslations with unprefixedPath', () => {
    ...
    it('should provide correct unprefixedPath to transform when config.path is set', () => {
        const config: LangTagTranslationsConfig = {
            namespace: 'test',
            path: 'my.prefix',
        };
        ...
        const callable: CallableTranslations<TestTranslationsType> =
    ...
    it('should provide correct unprefixedPath to processKey when config.path is set', () => {
        ...
        const processKeyMock = vi.fn(
            (
                context: TranslationKeyProcessorContext<typeof config>,
...
describe('lookupTranslation with unprefixed paths', () => {
    ...
    const strategy: TranslationMappingStrategy<LangTagTranslationsConfig> = {
        transform: ({ value, params }) =>
            value.replace('{{name}}', params?.name || ''), // Simplified transform
    };

    it('should lookup translation using unprefixed path when config.path is set', () => {
        const configWithPrefix: LangTagTranslationsConfig = {
            namespace: 'app',
            path: 'my.app',
        };
        const callable = createCallableTranslations(
    ...
    it('should return null when looking up with a prefixed path if translations are keyed unprefixedly', () => {
        ...
        const greetingFn = lookupTranslation(
            callable,
            'my.app.user.profile.greeting'
        );
    ...
    it('should return null for non-existent path', () => {
        ...
        const nonExistentFn = lookupTranslation(
            callable,
