...
const mockConfigWithPath: LangTagTranslationsConfig = { path: 'my.path', namespace: 'orders' };
...
describe('createCallableTranslations', () => {
    it('should transform a simple translation object, applying path from config', () => {
        ...
        const result = createCallableTranslations(translations, mockConfigWithPath, strategy);
    ...
    it('should not prefix keys when the config path is empty or undefined', () => {
        ...
        const result1 = createCallableTranslations(translations, { path: '', namespace: 'test' }, strategy);
        expect(result1.foo()).toBe('foo');

        const result2 = createCallableTranslations(translations, { namespace: 'test' }, strategy);
    ...
    it('should correctly use processKey to create an alias for keys ending with _other', () => {
        ...
        const keyProcessor: TranslationKeyProcessor<LangTagTranslationsConfig> = (
            ...
            if (context.key.endsWith('_other')) {
                const baseKey = context.key.slice(0, context.key.lastIndexOf('_other'));
                addProcessedKey(baseKey, sourceTranslations[context.key as keyof typeof sourceTranslations]); 
            }
        };

        const strategy: TranslationMappingStrategy<LangTagTranslationsConfig> = {
            transform: ({ value, params }) => {
                if (params && params.count) return `${value} (count: ${params.count})`;
                return value;
            },
            processKey: keyProcessor
        };
        type ExpectedResultType = CallableTranslations<typeof sourceTranslations> & { plural_key?: ParameterizedTranslation };
        const result = createCallableTranslations(sourceTranslations, { namespace: 'test' }, strategy) as ExpectedResultType;
        ...
        expect(result.plural_key_other({count: 5})).toBe('many items (count: 5)');

        expect(result.plural_key).toBeDefined();
        expect(result.plural_key!()).toBe('many items'); 
        expect(result.plural_key!({count: 10})).toBe('many items (count: 10)');
    });
    ...
    it('processKey should handle nested structures correctly', () => {
        ...
        const keyProcessor: TranslationKeyProcessor<LangTagTranslationsConfig> = (
            ...
            if (context.key.endsWith('_other')) {
                const shorten = context.key.slice(0, context.key.lastIndexOf('_other'));
        ...
        const strategy: TranslationMappingStrategy<LangTagTranslationsConfig> = {
            transform: ({ key, value, params }) => {
                if (params?.count && typeof value === 'string') { return value.replace('{{count}}', String(params.count)); }
                return value;
            },
            processKey: keyProcessor
        };
        ...
        type ExpectedNestedResult = CallableTranslations<typeof sourceTranslations.deeperSection> & {
        ...
        const result = createCallableTranslations(sourceTranslations, { path: 'app', namespace: 'inventory' }, strategy) as ExpectedFullResult;
        
        expect(result.deeperSection.status_available_one()).toBe('1 available');
        expect(result.deeperSection.status_available_other({ count: 5 })).toBe('5 available');
        expect(result.deeperSection.simpleKey()).toBe('simple value');

        expect(result.deeperSection.status_available).toBeDefined();
        expect(result.deeperSection.status_available!({ count: 10 })).toBe('10 available');
    });
    ...
    it('should correctly use params in transform function', () => {
        ...
        const strategy: TranslationMappingStrategy<LangTagTranslationsConfig> = {
            transform: ({ value, params }) => 
                value.replace(/{{(.*?)}}/g, (_, key) => params?.[key.trim()] || '')
        };
        const result = createCallableTranslations(translations, mockConfigWithPath, strategy);
        expect(result.greeting({ name: 'Alice', city: 'Wonderland' })).toBe('Hello Alice from Wonderland!');
     ...
     it('should ensure basePath always ends with a dot if not empty', () => {
        ...
        const res1 = createCallableTranslations(translations, { path: 'custom', namespace: 'test' }, strategy);
        expect(res1.key()).toBe('custom.key');

        const res2 = createCallableTranslations(translations, { path: 'custom.', namespace: 'test' }, strategy);
        expect(res2.key()).toBe('custom.key');

        const res3 = createCallableTranslations(translations, { path: '', namespace: 'test' }, strategy);
